    ...
    events: {
      ['element.example.reload'] () {
        this.$nextTick(() => {
          ...
          this.scrollWrap = document.querySelector('.app__content')
    ...
    methods: {
      ...
      scrollActiveCurrent () {
        ...
        anchors.forEach(function (item) {
          distances.push(item.offsetTop)
        })

        // let current = distances[index]
        // let max = distances.length - 1
        this.active = index
...
.toc .item .toc__link:hover,
.toc .item .toc__link.active {
	color: #216fc1;
}
